...
function AddMentorModal({ onClose, onCreated }) {
  const [form, setForm] = useState({ icon: '', title: '', subtitle: '', description: '', tag: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    try {
      const created = await apiPost('/mentors/', {
        icon: form.icon || null,
        title: form.title,
      ...
      setError(err instanceof ApiError ? err.message : 'Could not save this mentor. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <form className="space-y-md" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 gap-sm">
            <div className="space-y-xs">
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full bg-surface-container border border-outline-variant rounded-lg px-md py-sm font-body-md focus:border-secondary focus:ring-2 focus:ring-secondary/20 outline-none transition-all"
                placeholder="e.g. Software Engineer, Accra"
              />
            </div>
            <div className="space-y-xs">
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full bg-surface-container border border-outline-variant rounded-lg px-md py-sm font-body-md focus:border-secondary focus:ring-2 focus:ring-secondary/20 outline-none transition-all"
                placeholder="e.g. Matched or Suggested"
              />
            </div>
          </div>
          ...
          <div className="space-y-xs">
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full bg-surface-container border border-outline-variant rounded-lg px-md py-sm font-body-md focus:border-secondary focus:ring-2 focus:ring-secondary/20 outline-none transition-all"
              placeholder="e.g. engineering (Material Symbols name)"
            />
          </div>
          ...
          <div className="space-y-xs">
            ...
            <textarea
              ...
              placeholder="Why this mentor is a good match..."
            />
          </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-outline-variant/40">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-10 text-center text-on-surface-variant">
                    No mentors yet. Click "Add Mentor" to add one.
                  </td>
                </tr>
              ) : (
                filtered.map((m) => (
                  <tr key={m.id} className="hover:bg-surface-container transition-colors">
                    <td className="px-6 py-4 font-label-md text-label-md text-on-surface">{m.title}</td>
                    <td className="px-6 py-4 text-on-surface-variant">{m.subtitle || '—'}</td>
                    <td className="px-6 py-4 text-on-surface-variant">{m.tag || '—'}</td>
